...
exports.create = function(coilAddress) {

    const data = Buffer.alloc(6);

    data.writeUInt8(FRAME_START_CHAR_TO_NIBE, 0);
    data.writeUInt8(CMD_MODBUS_READ_REQ, 1);
    data.writeUInt8(0x02, 2); // data length
    data.writeUInt16LE(coilAddress, 3);
    data.writeUInt8(Protocol.calculateChecksum(data, 0, 5), 5); // data length
