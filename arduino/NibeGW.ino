...
long ethernetInitCounter = 0;
long ethernetInitDelay = 10000;
...
void setup()
{
  ...
  Serial1.begin(9600); // separated (non RS-485) serial for debugging
...
void loop()
{
  ...
  if (!ethernetInitialized)
  {
    if (ethernetInitCounter > ethernetInitDelay)
    {
      DEBUG_PRINT(1, "Initializing Ethernet\n");
      if (!initializeEthernet()) {
        ethernetInitCounter = 0;
        ethernetInitDelay = 60000;
        DEBUG_PRINT(1, "Ethernet init failed. Trying again soon\n");
...
int initializeEthernet()
{
  ...
  ethernetInitialized = Ethernet.begin(mac, 15000, 3000);
